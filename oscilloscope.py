...
class oscilloscope:
    ...
    def plotGraph(self):
        self.plt.clear()  #Must clear plot first or else plot is simply added
        self.plt.plot(self.t,self.ch1,'y',self.t,self.ch2,'c')  
        self.plt.set_xticklabels([])  #No labels on the axes
        self.plt.set_yticklabels([])
        self.plt.set_xticks(self.xticks,minor=False) #Major ticks to match scope display
        self.plt.set_yticks(self.yticks,minor=False)
        self.plt.set_xlim(0,2500) #Time scale is just sample number 
        self.plt.set_ylim(-96,96) #Data y range is actually -128 to 128 but this is to match the scope display   
